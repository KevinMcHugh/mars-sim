...
func (w *World) shoot(colonist, alien *Entity, weapon ItemKind, spec weaponSpec) {
	...
	if fatal {
		...
		o.ActorText = fmt.Sprintf("Killed an alien with a %s!", weapon)
		o.WitnessText = fmt.Sprintf("Watched %s kill an alien.", colonist.displayName())
		w.addGore(alien.Pos)
	...
	o.ActorText = fmt.Sprintf("Shot an alien in the %s with a %s.", part, weapon)
	o.WitnessText = fmt.Sprintf("Watched %s fight off an alien.", colonist.displayName())
	w.emitOccurrence(o)
