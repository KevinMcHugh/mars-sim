...
// ObjectRelationID is an observer-relative relationship to an occurrence's
// object. Empty means no configured relation applies.
type ObjectRelationID string

const ObjectRelationFriend ObjectRelationID = "friend"
...
func (p PerceptPattern) matches(percept Percept) bool {
	return p.OccurrencePattern.matches(percept.Occurrence) &&
		...
		(p.ObjectRelation == "" || p.ObjectRelation == percept.ObjectRelation)
}
...
func patternsOverlap(a, b PerceptPattern) bool {
	return idsOverlap(string(a.ActorNoun), string(b.ActorNoun)) &&
		...
		idsOverlap(string(a.ObjectRelation), string(b.ObjectRelation))
}
...
func (w *World) perceptFor(observer *Entity, channel ChannelID, role ObserverRole, phase PerceptPhase, o Occurrence) Percept {
	p := Percept{Observer: observer.ID, Channel: channel, Role: role, Phase: phase, Occurrence: o}
	if o.Object.Noun == NounColonist && o.Object.Entity != 0 && o.Object.Entity != observer.ID &&
		w.affinityBetween(observer.ID, o.Object.Entity) >= w.cfg.MoodFriendAffinity {
		p.ObjectRelation = ObjectRelationFriend
	}
	return p
}
	...
	for i := range w.cognition.Perceptions {
		...
		case ChannelDirect:
			...
			if observer := w.entities[id]; observer != nil && observer.Kind == Colonist && observer.Alive() {
				w.rememberPercept(observer, w.perceptFor(observer, rule.Channel, rule.Role, PhaseInstant, o))
			}
		default:
...
func (w *World) observePersistent(observer *Entity, only NounID) {
	...
	exiting := make([]perceptionKey, 0)
	for key := range observer.perceiving {
		if _, stillPresent := current[key]; stillPresent {
			continue
		}
		exiting = append(exiting, key)
	}
	sort.Slice(exiting, func(i, j int) bool {
		if exiting[i].Rule != exiting[j].Rule {
			return exiting[i].Rule < exiting[j].Rule
		}
		if exiting[i].Source != exiting[j].Source {
			return exiting[i].Source < exiting[j].Source
		}
		return exiting[i].Noun < exiting[j].Noun
	})
	for _, key := range exiting {
		occurrence := observer.perceiving[key]
		if rule := w.perceptionRule(key.Rule); rule != nil {
			w.rememberPercept(observer, w.perceptFor(observer, rule.Channel, rule.Role, PhaseExit, occurrence))
		}
	}
...
func (w *World) notePersistentPercept(observer *Entity, rule *PerceptionRule, o Occurrence, current map[perceptionKey]Occurrence) {
	...
	w.rememberPercept(observer, w.perceptFor(observer, rule.Channel, rule.Role, phase, o))
}
